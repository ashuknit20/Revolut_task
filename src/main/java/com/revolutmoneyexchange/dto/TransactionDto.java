...
import com.revolutmoneyexchange.db.H2DataSource;
import com.revolutmoneyexchange.exceptions.ImpossibleOperationExecution;
import com.revolutmoneyexchange.exceptions.ObjectMisMatchException;
import com.revolutmoneyexchange.model.*;
...
public class TransactionDto {
     ...
     *
     * @throws ObjectMisMatchException will be thrown if balance of the customer will be not enough for the moment.
     */
    public Transaction createTransaction(Transaction transaction) throws ObjectMisMatchException {
        ...
        try {
            ...
            if (fromBankAccount.getBalance().subtract(fromBankAccount.getBlockedAmount())
                ...
                throw new ObjectMisMatchException(ExceptionType.BAD_REQUEST_OBJECT,
            ...
            if (transaction == null) {
                throw new ObjectMisMatchException(ExceptionType.CANNOT_GENERATE_ID);
            }
     ...
     * @throws ObjectMisMatchException if provided ID will be null or transaction with that ID will be already
     * executed somehow
     */
    public void executeTransaction(Long id) throws ObjectMisMatchException {
        if (id == null) {
            throw new ObjectMisMatchException(ExceptionType.BAD_REQUEST_OBJECT,
     ...
     * @throws ObjectMisMatchException if transaction with the provided id will not be exists in the database at the
     * moment
     */
    private void updateTransaction(Transaction transaction) throws ObjectMisMatchException {
     ...
     * @param con the <code>Connection</code> to be used for this query
     */
    private void updateTransaction(Transaction transaction, Connection con) throws ObjectMisMatchException {
        ...
        DbUtils.QueryExecutor<Integer> queryExecutor = updateTransaction -> {
            ...
            updateTransaction.setLong(4, transaction.getAccountNumber());
        ...
        if (result == 0) {
            throw new ObjectMisMatchException(ExceptionType.RECORD_NOT_FOUND);
        }
    }
     ...
     *
     * @throws ObjectMisMatchException in case of any invalid parameter
     */
    private void verify(Transaction transaction) throws ObjectMisMatchException {
        if (transaction.getAmount() == null || transaction.getFromBankAccountId() == null ||
            ...
            throw new ObjectMisMatchException(ExceptionType.BAD_REQUEST_OBJECT, "Fields could not be NULL");
        }
    }
    ...
    private Transaction extractTransactionFromResultSet(ResultSet transactionsRS) throws SQLException {
        ...
        transaction.setAccountNumber(transactionsRS.getLong(TRANSACTION_ID_ROW));
